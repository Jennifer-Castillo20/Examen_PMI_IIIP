...
public class Adaptador extends ArrayAdapter<OBJ_foto> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        textView.setText(listado.get(position).getorden());
        return vis;
    }
    public void buscarcontact(String texto) {
        ...
        if (texto.length() == 0) {
            ...
            for (OBJ_foto contacto : copyContactos) {
                if (contacto.getorden().toLowerCase().contains(texto.toLowerCase())) {//si contacto en nombre contiene lo mismo que el texto
    ...
    public ArrayList<Contacto> Buscar_result_lista(String texto) {
        ...
        if (texto.length() == 0) {
            for (OBJ_foto contacto : copyContactos) {
                if (contacto.getorden().toLowerCase().contains(texto.toLowerCase())) {
                    Contacto contactoLista = new Contacto();
                    contactoLista.setId(String.valueOf(contacto.getId()));
                    contactoLista.setOrden(contacto.getorden());
                    contactoLista.setFoto(contacto.getImagen().toString());
                    contactoLista.setdescripcion(contacto.getDescripcion());
                    contactoLista.setPeriodista(contacto.getPeriodista());
